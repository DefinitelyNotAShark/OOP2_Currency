...
namespace Currency_Object_Midterm_Test
{
    ...
    public class USCurrencyRepoTests
    {
        ...
        public void AddCoinTest()
        {
            ...
            Assert.AreEqual(Math.Round(valueOrig + penny.MonetaryValue, 2), valueAfterPenny);
            Assert.AreEqual(Math.Round(valueAfterPenny + (numPennies * penny.MonetaryValue),2), valueAfterFiveMorePennies);

            Assert.AreEqual(Math.Round(valueAfterFiveMorePennies + nickel.MonetaryValue, 2), valueAfterNickel);
            Assert.AreEqual(Math.Round(valueAfterNickel + dime.MonetaryValue, 2), valueAfterDime);
            Assert.AreEqual(Math.Round(valueAfterDime + quarter.MonetaryValue,2), valueAfterQuarter);
            Assert.AreEqual(Math.Round(valueAfterQuarter + dollarCoin.MonetaryValue, 2), valueAfterDollar);
        ...
        public void RemoveCoinTest()
        {
            ...
            Assert.AreEqual(Math.Round(valueAfterFiveMorePennies - nickel.MonetaryValue, 2), valueAfterNickel); //HUH? 1.35 != 1.35 both are double?
            Assert.AreEqual(Math.Round(valueAfterNickel - dime.MonetaryValue, 2), valueAfterDime);
            Assert.AreEqual(Math.Round(valueAfterDime - quarter.MonetaryValue, 2), valueAfterQuarter);
            Assert.AreEqual(Math.Round(valueAfterQuarter - dollarCoin.MonetaryValue, 2), valueAfterDollar);
        }
